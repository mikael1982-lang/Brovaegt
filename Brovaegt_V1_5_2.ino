...
const float VEHICLE_PRESENT_WEIGHT = 0.500;
...
void handleVehicleSave(){if(!server.hasArg("epc")||!server.hasArg("name")||!server.hasArg("tare")){server.send(400,"text/plain","Mangler RFID, navn eller tomvægt.");return;}String epc=cleanText(server.arg("epc"));epc.toUpperCase();String name=cleanText(server.arg("name"));String tareText=cleanText(server.arg("tare"));tareText.replace(",",".");if(epc.length()==0||name.length()==0||tareText.length()==0){server.send(400,"text/plain","Udfyld RFID, navn og tomvægt.");return;}float tareWeight=tareText.toFloat();if(tareWeight<0.0){server.send(400,"text/plain","Tomvægt må ikke være negativ.");return;}int index=findVehicleIndex(epc);if(index<0){if(vehicleCount>=MAX_VEHICLES){server.send(400,"text/plain","Registeret er fuldt (maks. 20 køretøjer).");return;}index=vehicleCount++;}vehicles[index].epc=epc;vehicles[index].name=name;vehicles[index].tare=tareWeight;saveVehicles();if(identifiedEpc==epc){identifiedVehicleName=name;identifiedVehicleTare=tareWeight;identifiedVehicleKnown=true;}Serial.print("VEHICLE SAVED: ");Serial.print(epc);Serial.print(" / ");Serial.print(name);Serial.print(" / ");Serial.println(tareWeight,3);server.send(200,"text/plain","OK");}
...
void processRfidTagFrame(){int epcBytes=(((rfidFrame[6]<<8)|rfidFrame[7])>>11&0x1F)*2;if(epcBytes<=0||8+epcBytes>rfidFrameLength-4)return;String epc="";for(int i=0;i<epcBytes;i++){if(rfidFrame[8+i]<0x10)epc+="0";epc+=String(rfidFrame[8+i],HEX);}epc.toUpperCase();if(epc!=lastRfidEpc){Serial.print("[");Serial.print(millis());Serial.print(" ms] RFID TAG: ");Serial.println(epc);lastRfidEpc=epc;}identifiedEpc=epc;identifiedVehicleKnown=lookupVehicle(epc,identifiedVehicleName,identifiedVehicleTare);if(!identifiedVehicleKnown){identifiedVehicleName="UKENDT";identifiedVehicleTare=0.0;}rfidSearching=false;if(!rfidStopSent){Serial2.write(STOP_MULTI_CMD,sizeof(STOP_MULTI_CMD));Serial.print("[");Serial.print(millis());Serial.println(" ms] STOP POLL SENT");rfidStopSent=true;}}

void handleRfidInput(){while(Serial2.available()>0){byte rfidByte=Serial2.read();if(rfidFrameLength==0){if(rfidByte==0xAA)rfidFrame[rfidFrameLength++]=rfidByte;continue;}if(rfidByte==0xAA){rfidFrame[0]=rfidByte;rfidFrameLength=1;continue;}if(rfidFrameLength>=RFID_MAX_FRAME_LENGTH){resetRfidFrame();continue;}rfidFrame[rfidFrameLength++]=rfidByte;if(rfidFrameLength>=5){size_t expectedLength=((size_t)rfidFrame[3]<<8|rfidFrame[4])+7;if(expectedLength>RFID_MAX_FRAME_LENGTH)resetRfidFrame();else if(rfidFrameLength==expectedLength){if(rfidFrame[rfidFrameLength-1]==0xDD&&rfidFrame[1]==0x02&&rfidFrame[2]==0x22&&rfidFrameChecksumValid())processRfidTagFrame();resetRfidFrame();}else if(rfidFrameLength>expectedLength)resetRfidFrame();}}}

void updateVehicleCycle(){
  if(!vehicleWasOnScale && currentWeight>=VEHICLE_PRESENT_WEIGHT){
    vehicleWasOnScale=true;
    vehicleLeftTimer=0;
    if(identifiedEpc.length()==0){
      startRfidSearch();
...
void setup(){Serial.begin(115200);scale.begin(DOUT,SCK);delay(1000);tare();loadVehicles();Serial.print("VEHICLES LOADED: ");Serial.println(vehicleCount);Serial2.begin(115200,SERIAL_8N1,RFID_RX_PIN,RFID_TX_PIN);rfidSearching=false;WiFi.mode(WIFI_AP);WiFi.softAP(ssid,password);server.on("/",handleRoot);server.on("/weight",handleWeight);server.on("/vehicles",HTTP_GET,handleVehicles);server.on("/vehicle/save",HTTP_POST,handleVehicleSave);server.on("/vehicle/delete",HTTP_POST,handleVehicleDelete);server.begin();Serial.print("AP IP: ");Serial.println(WiFi.softAPIP());Serial.println("RFID READY - WAITING FOR 0.500 kg");}

void loop(){handleRfidInput();long raw=averageRead(20);currentWeight=(offset-raw)/calibrationFactor;if(fabs(currentWeight-lastWeight)<0.005){if(stableTimer==0)stableTimer=millis();if(millis()-stableTimer>2000){stable=true;lastStableWeight=currentWeight;}}else{stable=false;stableTimer=0;}updateVehicleCycle();handleRfidPolling();lastWeight=currentWeight;server.handleClient();}
