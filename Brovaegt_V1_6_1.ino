...
void loadVehicles(){
  ...
  for(int i=0;i<vehicleCount;i++){
    ...
    String tKey="t"+String(i), rKey="r"+String(i), gKey="g"+String(i);
...
void saveVehicles(){
  ...
  for(int i=0;i<MAX_VEHICLES;i++){
    ...
    if(i<vehicleCount){
      ...
      preferences.remove(tKey.c_str()); preferences.remove(rKey.c_str()); preferences.remove(gKey.c_str());
    }
  }
}
...
float calculatedRealGross(){ return identifiedRealTare+calculatedRealLoad(); }
...
<div id='result' class='unknown'>VENTER PÅ KØRETØJ</div><div id='overBy' class='small'></div><a class='adminLink' href='/vehicles'>Køretøjsregister</a>
<script>
...
<div class='weightRow'><div class='weightField'><label>Model egenvægt (måles af brovægten)</label><input id='regModelTare' type='text' readonly placeholder='Vent på stabil vægt'></div><div class='weightButton'><button id='useWeight' class='capture' type='button'>Brug aktuel vægt</button></div></div>
<div class='hint'>Køretøjet skal stå tomt på broen, og vægten skal være STABIL.</div>
...
function captureWeight(){setMessage('Kontrollerer vægten...');ajax('GET','/current-weight',null,function(t){var p=t.split(';');if(p[0]!='1'){setMessage('Vent på STABIL vægt.');return}var w=Number(p[1]);if(isNaN(w)||w<0.500){setMessage('Der skal stå et køretøj på broen (mindst 0,500 kg).');return}byId('regModelTare').value=w.toFixed(3);setMessage('Model egenvægt målt: '+w.toFixed(3)+' kg')},function(e){setMessage(e)})}
function editVehicle(v){byId('regEpc').value=v.epc;byId('regName').value=v.name;byId('regOwner').value=v.owner;byId('regModelTare').value=Number(v.modelTare).toFixed(3);byId('regRealTare').value=Math.round(Number(v.realTare));byId('regMaxGross').value=Math.round(Number(v.maxGross));setMessage('Køretøj klar til redigering. Brug aktuel vægt igen, hvis model-egenvægten skal måles på ny.');window.scrollTo(0,0)}
...
void handleStatus(){
  ...
  server.send(200,"text/plain",
    String(identifiedVehicleKnown?1:0)+";"+String(onScale?1:0)+";"+String(stable?1:0)+";"+
    (identifiedVehicleKnown?identifiedVehicleName:"")+";"+(identifiedVehicleKnown?identifiedVehicleOwner:"")+";"+
    String(identifiedVehicleKnown?identifiedRealTare:0.0,0)+";"+String(identifiedVehicleKnown?load:0.0,0)+";"+
    String(identifiedVehicleKnown?gross:0.0,0)+";"+String(identifiedVehicleKnown?identifiedMaxGross:0.0,0)+";"+String(overload?1:0));
}

void handleCurrentRfid(){ server.send(200,"text/plain",identifiedEpc); }

void handleCurrentWeight(){
  server.send(200,"text/plain",String(stable?1:0)+";"+String(currentWeight,3));
}
...
void handleVehiclesJson(){
  ...
  for(int i=0;i<vehicleCount;i++){
    if(i>0) json+=",";
    json+="{\"epc\":\""+jsonEscape(vehicles[i].epc)+"\",\"name\":\""+jsonEscape(vehicles[i].name)+"\",\"owner\":\""+jsonEscape(vehicles[i].owner)+"\",\"modelTare\":"+String(vehicles[i].modelTare,3)+",\"realTare\":"+String(vehicles[i].realTare,0)+",\"maxGross\":"+String(vehicles[i].maxGross,0)+"}";
  }
  json+="]";
...
void handleVehicleSave(){
  if(!server.hasArg("epc")||!server.hasArg("name")||!server.hasArg("owner")||!server.hasArg("modelTare")||!server.hasArg("realTare")||!server.hasArg("maxGross")){
  ...
  int index=findVehicleIndex(epc);
  if(index<0){ if(vehicleCount>=MAX_VEHICLES){ server.send(400,"text/plain","Registeret er fuldt (maks. 20 køretøjer)."); return; } index=vehicleCount++; }
  vehicles[index].epc=epc; vehicles[index].name=name; vehicles[index].owner=owner; vehicles[index].modelTare=modelTare; vehicles[index].realTare=realTare; vehicles[index].maxGross=maxGross;
  saveVehicles();
  if(identifiedEpc==epc) identifiedVehicleKnown=lookupVehicle(epc);
  Serial.print("VEHICLE SAVED: "); Serial.print(epc); Serial.print(" / "); Serial.print(name); Serial.print(" / "); Serial.print(owner); Serial.print(" / model "); Serial.print(modelTare,3); Serial.print(" kg / real "); Serial.print(realTare,0); Serial.print(" kg / max "); Serial.println(maxGross,0);
...
void handleVehicleDelete(){
  ...
  vehicleCount--; saveVehicles();
  if(identifiedEpc==epc){ identifiedVehicleKnown=false; identifiedVehicleName="UKENDT"; identifiedVehicleOwner=""; identifiedModelTare=0.0; identifiedRealTare=0.0; identifiedMaxGross=0.0; }
...
void startRfidSearch(){
  rfidSearching=true; rfidStopSent=false; lastRfidEpc=""; lastRfidPoll=0; resetRfidFrame();
  Serial.println("RFID SEARCH STARTED - WEIGHT >= 0.500 kg");
}

void stopRfidSearchSilently(){ rfidSearching=false; rfidStopSent=false; lastRfidPoll=0; resetRfidFrame(); }

void handleRfidPolling(){
  if(!rfidSearching) return;
  if(lastRfidPoll==0 || millis()-lastRfidPoll>=RFID_POLL_INTERVAL){ Serial2.write(SINGLE_POLL_CMD,sizeof(SINGLE_POLL_CMD)); lastRfidPoll=millis(); }
}
...
void processRfidTagFrame(){
  ...
  if(!identifiedVehicleKnown){ identifiedVehicleName="UKENDT"; identifiedVehicleOwner=""; identifiedModelTare=0.0; identifiedRealTare=0.0; identifiedMaxGross=0.0; }
  rfidSearching=false;
...
void updateVehicleCycle(){
  if(!vehicleWasOnScale && currentWeight>=RFID_TRIGGER_WEIGHT){ vehicleWasOnScale=true; vehicleLeftTimer=0; startRfidSearch(); }
  if(vehicleWasOnScale && currentWeight<VEHICLE_LEFT_WEIGHT){
    if(vehicleLeftTimer==0) vehicleLeftTimer=millis();
    if(millis()-vehicleLeftTimer>=VEHICLE_LEFT_DELAY){ clearIdentifiedVehicle(); vehicleWasOnScale=false; vehicleLeftTimer=0; stopRfidSearchSilently(); Serial.println("RFID READY - WAITING FOR 0.500 kg"); }
  } else if(currentWeight>=VEHICLE_LEFT_WEIGHT) vehicleLeftTimer=0;
}

void setup(){
  Serial.begin(115200); scale.begin(DOUT,SCK); delay(1000); tareScale();
  loadVehicles(); Serial.print("VEHICLES LOADED: "); Serial.println(vehicleCount);
  Serial2.begin(115200,SERIAL_8N1,RFID_RX_PIN,RFID_TX_PIN); stopRfidSearchSilently(); Serial.println("RFID READY - WAITING FOR 0.500 kg");
...
void loop(){
  handleRfidInput(); handleRfidPolling();
  ...
  } else { stable=false; stableTimer=0; }
  updateVehicleCycle(); lastWeight=currentWeight; server.handleClient();
}
